...
// begin qworum-messages-*.mjs
// TODO use qworum-messages without the XML part; use json encodings to communicate with the browser extension.
...
class Goto extends Instruction {
      ...
      try {
          ...
          for (const parametersElement of element.children){
              ...
              try {
                  ...
                  if (!(new URL(parametersNamespace).href === Goto.namespace.href && parametersTag === 'parameters')) throw `not a parameters`;
                  for (const parameterElement of parametersElement.children){
                      ...
                      try {
                          ...
                          if (!(new URL(parameterNamespace).href === Goto.namespace.href && parameterTag === 'parameter')) throw `not a parameter`;
  ...
  toXmlElement(namespaceStack) {
      ...
      if (this.parameters.length > 0) {
          nsStack.push(element);
          const parametersAttributes = Object.create(null), parametersChildren = [], parametersElement = new XmlElement(XmlNamespaceStack.elementName('parameters', prefix), parametersAttributes, parametersChildren);
          element.children.push(parametersElement);
          for (const parameter of this.parameters){
              const parameterAttributes = Object.create(null), parameterChildren = [], parameterElement = new XmlElement(XmlNamespaceStack.elementName('parameter', prefix), parameterAttributes, parameterChildren);
...
class Call extends Instruction {
      ...
      try {
          ...
          for (const objectParametersElement of element.children){
              ...
              try {
                  ...
                  if (!(new URL(objectParametersNamespace).href === Call.namespace.href && objectParametersTag === 'object-parameters')) throw `not an object parameters element`;
                  for (const objectParameterElement of objectParametersElement.children){
                      ...
                      try {
                          ...
                          if (!(new URL(objectParameterNamespace).href === Call.namespace.href && objectParameterTag === 'object-parameter')) throw `not an object parameter`;
          ...
          for (const parametersElement of element.children){
              ...
              try {
                  ...
                  const parametersNamespace = nsStack.findNamespace(parametersElement.name), parametersTag = XmlNamespaceStack.getElementNameWithoutPrefix(parametersElement.name);
                  if (!parametersNamespace) throw new Error(`not a namespace`);
                  if (!(new URL(parametersNamespace).href === Call.namespace.href && parametersTag === 'parameters')) throw `not a parameters element`;
                  ...
                  for (const parameterElement of parametersElement.children){
                      ...
                      try {
                          ...
                          const parameterNamespace = nsStack.findNamespace(parameterElement.name), parameterTag = XmlNamespaceStack.getElementNameWithoutPrefix(parameterElement.name);
                          if (!parameterNamespace) throw new Error(`not a namespace`);
                          if (!(new URL(parameterNamespace).href === Call.namespace.href && parameterTag === 'parameter')) throw `not a parameter`;
                          if (!(typeof parameterElement.attributes.name === 'string')) throw new Error('parameter without name');
                          const parameterName = parameterElement.attributes.name;
                          for (const parameterValueElement of parameterElement.children){
                              if (parameterValueElement.type !== XmlNode.TYPE_ELEMENT) continue;
  ...
  toXmlElement(namespaceStack) {
      ...
      if (this.objectParameters.length > 0) {
          nsStack.push(element);
          const objectParametersAttributes = Object.create(null), objectParametersChildren = [], objectParametersElement = new XmlElement(XmlNamespaceStack.elementName('object-parameters', prefix), objectParametersAttributes, objectParametersChildren);
          element.children.push(objectParametersElement);
          for (const objectParameter of this.objectParameters){
              const objectParameterAttributes = Object.create(null), objectParameterChildren = [], objectParameterElement = new XmlElement(XmlNamespaceStack.elementName('object-parameter', prefix), objectParameterAttributes, objectParameterChildren);
...
class PhaseParameters {
  ...
  static fromXml(xmlStr) {
      try {
          ...
          if (!(elementNs === PhaseParameters.namespace.href && elementName === 'parameters')) throw new Error('not a valid phase-parameters message');
          for (const paramElement of doc.root.children){
              ...
              if (!(paramElementNs === PhaseParameters.namespace.href && paramElementName === 'parameter')) throw new Error('not a param');
  ...
  toXml() {
      ...
      }, children = [], params = new XmlElement('parameters', attributes, children), nsStack = new XmlNamespaceStack();
      nsStack.push(params);
      for (const param of this.parameters){
          const data = param.value, paramElement = new XmlElement('parameter', {
              name: param.name
          }, [
...
class Qworum {
   ...
   * @param {(object | object[] | null | undefined)} parameters - Named data value parameters.
   * @param {boolean} sendParameters - If true, send parameters in the HTTP(S) request's body. Default is false.
   * @param {(object | object[] | null | undefined)} objectParameters - Named Qworum object parameters.
   * @throws {Error}
